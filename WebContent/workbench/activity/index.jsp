...
<!-- 导入文件必须先导入jquery，bootstrap里面的有基于jquery文件 -->
				...
				function() {
					...
					$.ajax({
						...
						success : function(data) {
							...
							//在js中使用el表达式，el表达式一定要用在字符串里面
							var id = "${user.id}";

							//让下拉框默认选中登录的用户
							$("#create-owner").val(id);
							//操作模态窗口的方式
							//需要获取需要操作的模态窗口的jquery对象，调用modal方法，方法传递参数show：打开模态窗口 hide：关闭模态窗口
					...
					$.ajax({
						...
						success : function(data) {
							if(data.success){
								...
								/*
									不需要执行什么操作，只需要直接使用就行
									$("#activityPage").bs_pagination('getOption','currentPage')表示操作后停留到当前页
									$("#activityPage").bs_pagination('getOption','rowsPerPage')表示操作后维持当前展示的记录数
								*/
								pageList(1,$("#activityPage").bs_pagination('getOption','rowsPerPage'));

								
								//清除模糊态窗口
								//jquery没有提供表单的reset方法，原生的js有提供原生的reset方法
								/*
									jquery对象转换为dom对象
										jquery对象[下标]
									dom对象转换为jquery对象
			...
			$("#editBtn").click(function(){
				...
				$.ajax({
					...
					success:function(data){
						...
						$("#edit-id").val(data.a.id);//用于前端返回修改的数据
	...
	function pageList(pageNo,pageSize){
		...
		$.ajax({
			...
			success:function(data){
				...
				//数据处理完毕后结合分页插件，对前端展示分页相关的信息
				
				$("#activityPage").bs_pagination({
					currentPage:pageNo,    //页码
					rowsPerPage:pageSize,  //每页显示的记录条数
					maxRowsPerPages:20	, //每页最多的记录条数
				...
				<div class="modal-body">
					...
					<form class="form-horizontal" role="form">
						...
						<div class="form-group">
							...
							<div class="col-sm-10" style="width: 81%;">
								<!-- 
								1.textarea标签对要紧紧的挨着，空格也算是内容
								2.textarea虽然是以标签对的形式来呈现的，但是是属于表单元素的范畴，所有对textarea的取值和赋值操作一般是以val（）方法（不是html()方法）
								 -->
		...
		<div style="width: 100%; position: absolute; top: 5px; left: 10px;">
			...
			<div class="btn-toolbar" role="toolbar"
				...
				<div class="btn-group" style="position: relative; top: 18%;">
					<!--  
					data-toggle="modal"表示将要打开一个模糊态窗口
					data-target="#createActivityModal"表示打开的目标窗口
					<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#createActivityModal">
					以属性和属性值的方式写在了button元素中，存在问题
					问题在于没有办法对该按钮进行扩充
					
					将来开发时对于触发模态窗口的操作，一定不要写死在元素中
				 -->
